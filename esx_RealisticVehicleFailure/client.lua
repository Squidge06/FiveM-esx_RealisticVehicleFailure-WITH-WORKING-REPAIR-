	...
	if isPedDrivingAVehicle() then
		...
		if IsNearMechanic() then
			if GetIsVehicleEngineRunning(vehicle) == False then
				notification("~g~Engine must be turned on to repair")
				return
			else
				...
				if PlayerData.job.name == 'mecano' then
					...
					if mechNumb == 1 then
						if GetIsVehicleEngineRunning(vehicle) == False then
							notification("~g~Engine must remain on for repair")
							return
						else
							SetVehicleUndriveable(vehicle,false)
							SetVehicleFixed(vehicle)
							...
							if cfg.chargeForRepairs then
								notification("~g~The Mechanic repaired your car for $" .. repairCost .. "!")
								return
							else
